  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        >
          <h2 className="font-[Georgia,serif] text-xl font-bold text-brand-dark">{product.name}</h2>
        </Link>
        ...
        <Link
          to={`/products/${product.slug}`}
          className="mt-5 inline-flex justify-center rounded-full bg-brand px-4 py-2 text-sm font-semibold text-white transition duration-200 hover:bg-brand-dark focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-2"
        >
          View Product
        </Link>
      </div>
